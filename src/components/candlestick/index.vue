...
export default {
  ...
  mounted: function() {
    ...
    const initTradingView = () => {
      this_vue.chart = window.tvWidget = new window.TradingView.widget({
        fullscreen: false,
        autosize: true,
    ...
    // 触发
    window.TradingView.onready(initTradingView)
    this.$eventBus.$emit('TradingViewOnready', initTradingView)
  },
  methods: {
    ...
    adjustChart: function() {
      const chart_iframe = $('#chart_container').find('iframe')
      const chart_top = chart_iframe.contents().find('.header-chart-panel')
      const chart_top_container = chart_iframe.contents().find('.header-chart-panel-content')
      const chart_bottom = chart_iframe.contents().find('.date-range-wrapper')
